 ...
 * 
 * Usage:
 *   node scripts/migratePurchases.mjs [--dry-run]
 * 
 * Options:
...
if (!getApps().length) {
    ...
    if (serviceAccountPath) {
        try {
            ...
            console.log('');
            console.log('Please set GOOGLE_APPLICATION_CREDENTIALS to your service account JSON file path:');
...
async function checkPurchaseExists(userId, contributionId) {
...
async function createPurchase(userId, contribution) {
    const purchasesRef = db.collection('users').doc(userId).collection('purchases');
...
async function migrateContributionsToPurchases(dryRun = false) {
